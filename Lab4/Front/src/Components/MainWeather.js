...
export class MainWeather extends React.Component {
	...
	getWeather = async (latitude, longitude) => {
		...
		*/
		/*
		const url = "http://localhost:3000/weather/coordinates?lat=" + latitude + "&long=" + longitude;
		$.ajax({
			url : url,
			type: "GET",
			success:this.setState({ loading: false, success: true, data: data })
		});
		*/
		...
		Http.onreadystatechange = function () {
			if (this.readyState === 4 && this.status === 200) {
				...
				comp.setState({ loading: false, success: true, data: transform(data) });
			}
		}
